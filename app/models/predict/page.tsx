        ...
        {showReport && predictions ? (
          <div className="flex flex-col min-h-[calc(100vh-2rem)] overflow-y-auto">
            <div className="flex items-center justify-between mb-4 bg-slate-50/80 backdrop-blur-sm sticky top-0 z-10 py-3 shrink-0 border-b border-slate-100">
              ...
              <button
                ...
                disabled={isSaving}
                className="flex items-center gap-2 bg-amber-600 text-white px-5 py-2.5 rounded-xl text-[10px] font-black hover:bg-amber-700 transition-all shadow-lg active:scale-95 disabled:opacity-50 uppercase tracking-widest"
              >
                <Activity size={14} />
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pb-8">
            <div className="mb-5 text-center">
              <h1 className="text-3xl md:text-4xl font-black text-amber-600 mb-1 tracking-tight">
                Basic Diagnosis
              </h1>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 mb-4">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full py-3 pl-11 pr-4 rounded-xl border border-amber-300 focus:border-amber-600 focus:ring-2 focus:ring-amber-100 outline-none shadow-sm text-amber-700 text-sm transition-all bg-white"
                />
                        ...
                        <div className="max-h-[240px] overflow-y-auto">
                          {searchResults.map((s) => (
                            <button
                              ...
                              className="w-full text-left px-3 py-2.5 hover:bg-amber-50 border-b border-slate-50 last:border-none transition-all flex justify-between items-center group"
                            >
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {selected.map((s) => (
                  <button
                    ...
                    className="group flex items-center gap-1.5 bg-amber-600 text-white pl-3 pr-2 py-1.5 rounded-lg text-xs font-bold shadow-sm hover:bg-amber-700 transition-all"
                  >
                    {formatName(s)}{" "}
                ...
                <div className="flex flex-wrap gap-1.5">
                  {related.map((s) => (
                    <button
                      key={s}
                      onClick={() => toggleSymptom(s)}
                      className="bg-white text-amber-700 border border-amber-200 px-3 py-1.5 rounded-md text-xs font-bold hover:bg-amber-600 hover:text-white transition-all"
                    >
                      + {formatName(s)}
                    </button>
                  ))}
                    ...
                    (s) =>
                      !selected.includes(s) && (
                        <button
                          key={s}
                          onClick={() => toggleSymptom(s)}
                          className="text-left px-3 py-2.5 rounded-xl bg-white border border-amber-200 text-amber-700 text-[11px] font-bold hover:border-amber-500 hover:bg-amber-50 transition-all truncate shadow-sm"
                        >
                          {formatName(s)}
                        </button>
                      ),
              ...
              <section className="pt-6 pb-3 border-t border-slate-200">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {displayedOthers.map((s) => (
                    <button
                      key={s}
                      onClick={() => toggleSymptom(s)}
                      className="inline-flex items-center px-3 py-2 rounded-lg bg-white border border-amber-300 text-amber-800 text-xs tracking-tight hover:border-amber-500 hover:bg-amber-50 transition-all whitespace-nowrap"
                    >
                      {formatName(s)}
                    </button>
                  ))}
                ...
                {otherSymptoms.length > 12 && (
                  <button
                    ...
                    className="w-full mt-4 py-2.5 text-[10px] font-black text-amber-600 uppercase tracking-widest bg-white border border-amber-200 rounded-xl hover:bg-amber-50 transition-colors"
                  >
                    {isExpanded
                      ? "Collapse ↑"
